...
import { myStorage, fixD } from '@/utils';
...
export default {
  ...
  data() {
    return {
      ...
      // --买单预估强评价
      liquidationBuyPrice: null,
      // --卖单预估强平价
      liquidationSellPrice: null,
  ...
  computed: {
    ...
    buttosContent() {
      return {
        buyButton: {
          // text: this.isLogin ? this.$t('contract.buyButton') : this.$t('contract.loginReg'),
          // class: this.isLogin ? 'u-1-bg buyBtn u-16-cl' : 'u-8-bg buyBtn u-16-cl',
          text: this.isLogin ? this.$t('personal.state.closed') : this.$t('contract.loginReg'),
          class: this.isLogin ? 'u-11-bg buyBtn u-16-cl' : 'u-8-bg buyBtn u-16-cl',
        },
        sellButton: {
          // text: this.isLogin ? this.$t('contract.sellButton') : this.$t('contract.loginReg'),
          // class: this.isLogin ? 'u-4-bg sellBtn u-16-cl' : 'u-8-bg sellBtn u-16-cl',
          text: this.isLogin ? this.$t('personal.state.closed') : this.$t('contract.loginReg'),
          class: this.isLogin ? 'u-11-bg sellBtn u-16-cl' : 'u-8-bg sellBtn u-16-cl',
        },
      };
    },
    // 当前币对 计价单位
  ...
  methods: {
    ...
    onChangelevel(data) {
      ...
      if (this.coBase && this.is_more_position !== 1) {
        this.sendLevelMultiple(data);
      } else {
        this.definiteLevel = `${data}`;
      ...
      if (this.isLogin) {
        ...
        this.axios({
          ...
          if (data.code === '0') {
            ...
            // --买单预估强评价
            this.liquidationBuyPrice = fixD(data.data.liquidationBuyPrice, this.pricePrecision);
            // --卖单预估强平价
            this.liquidationSellPrice = fixD(data.data.liquidationSellPrice, this.pricePrecision);
            // 设置默认杠杆倍数
    ...
    sendLevelMultiple(level) {
      ...
      this.axios({
        ...
        params: {
          ...
          leverageLevel: level,
          positionId: this.currentOrderData ? this.currentOrderData.id : '',
        },
      }).then((data) => {
        if (data.code === '0') {
