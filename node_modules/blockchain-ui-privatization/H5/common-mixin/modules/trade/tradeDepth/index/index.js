...
export default {
  ...
  computed: {
    sellList() {
      const { asksData } = this.dataList;
      const len = this.dataList.asksData.length;
      return asksData.length > 4 ? asksData.slice(len - 5) : asksData;
    },
    buyList() {
    ...
    theadList() {
      if (this.symbolCurrent) {
        ...
        return [
          `${this.$t('trade.price')}`,
          `${this.$t('trade.number')}`,
          // `${this.$t('trade.total')}(${this.symbolCurrent.split('/')[0]})`,
        ];
      }
      return [
        `${this.$t('trade.price')}`,
    ...
    depthOptionList() {
      const arr = [];
      this.depthOption.forEach((item, index) => {
        arr.push({ name: `${this.$t('h5Add.depth')}${item}`, index });
      });
      return arr;
    },
    // 深度选项
  ...
  methods: {
    ...
    setDepth(v) {
      this.currentdepthClass = v.index;
