import {
  colorMap, getHex, setBoxHeight, imgMap, getCookie, myStorage,
} from '@/utils';
...
export default {
  ...
  methods: {
      ...
      return {
        ...
        getBars(symbolInfo,
          ...
          self.$bus.$on('KLINE_DATA_REQ', (data) => {
            ...
            const symbolType = `${symbol}_${type}`;
            if (data.event_rep === 'rep' && symbolType === self.symbolName.symbol) {
        ...
        subscribeBars(symbolInfo, resolution, onRealtimeCallback) {
          ...
          self.$bus.$on('KLINE_DATA_SUB', (data) => {
            ...
            const [, symbol, type] = data.channel.split('_');
            const symbolType = `${symbol}_${type}`;
            if (data.tick && symbolType === self.symbolName.symbol && lastTimeS === Ntime) {
        ...
        unsubscribeBars(subscriberUID) {
          ...
          const lastTimeS = self.getTimeMin(true, parseFloat(arr[2]));
          self.$bus.$emit('WEBSOCKET_KLINE_SEND', {
            type: 'unsub',
            symbol,
            lastTimeS,
    ...
    fixDepthNumber(symbolName) {
      if (this.symbolAll) {
        const n = this.symbolAll[symbolName].pricePrecision;
        const b = 10 ** n;
    ...
    // 格式化数据
    setData(obj) {
      // console.log(formatTime(obj.id * 1000));
      return {
        time: obj.id * 1000,
        close: obj.close,
        open: obj.open,
