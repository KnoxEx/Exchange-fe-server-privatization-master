...
export default {
  ...
  actions: {
    ...
    // 获取 合约 copublic_info 数据
    getCoPublicInfo({ commit }) {
      axios({
        url: 'contract_public_info_v2',
        method: 'post',
        hostType: 'co',
      }).then((data) => {
        if (Number(data.code) === 0) {
          // 设置默认选中的币对
          if (data.data.switch && data.data.switch.is_more_position) {
            commit('ISMOREPOSITION', Number(data.data.switch.is_more_position));
          }
          setCoMarket(data.data.market, data.data.marketSymbol);
          commit('GETCOPUBLICINFO', data.data);
        }
      });
    },
