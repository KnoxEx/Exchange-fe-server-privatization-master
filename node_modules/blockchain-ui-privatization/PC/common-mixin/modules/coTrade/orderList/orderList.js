    ...
    columns() {
      if (this.orderType === 0) {
        ...
        if (this.is_more_position === 1) {
          arr.unshift({
            title: this.$t('contract.accounttId'), // 合约ID
            width: '100px',
          });
        }
        return arr;
  ...
  methods: {
              ...
              tableData = {
                ...
                data: [
                  ...
                  [
                    {
                      type: 'button',
                      text: this.$t('contract.cancel'), // 撤单
                      eventType: 'cancel',
    ...
    formPosition(data) {
      ...
      if (dataArray.length) {
        ...
        dataArray.forEach((item) => {
          if ((this.marketCurrent || myStorage.get('coNowSymbol')) === item.symbol && this.is_more_position !== 1) {
          ...
          if (this.cancelOrderId !== item.orderId) {
            ...
            // 如果是分仓模式，合约id
            if (this.is_more_position === 1) {
              arrData.unshift(item.id);
            }
            const tableData = {
              id: item.id,
              classes: this.currenSymboltDataId === item.id ? 'a-21-bg' : '',
              data: arrData,
            };
            newData.push(tableData);
          }
    ...
    handleElement(item) {
      ...
      if (this.is_more_position === 1) {
        arr.push({
          type: 'input',
          text: this.limitPriceObj[item.id], // '市价平仓',
          classes: 'a-3-bd b-1-cl',
          eventType: 'inputValueChane',
        });
        arr.push({
          type: 'button',
          text: this.$t('contract.limitPrice'), // '限价平仓',
          eventType: 'closeTwo',
        });
      }
      arr.push({
        type: 'button',
        text: this.$t('contract.closePrice'), // '市价平仓',
        eventType: 'close',
      });
      return arr;
    },
    ...
    unrealisedAmount(data) {
      const template = `<div class="hover">
        ...
        <span class="hover_show">
          ${fixD(data.unrealisedAmountMarket, data.valuePrecision)}
          ${data.bond}
          (${fixD(data.unrealisedRateMarket, 2)})
        </span>
      </div>`;
      return template;
    },
    ...
    realisedAmoun(data) {
      const template = `<div class="hover">
        ...
        <span class="hover_show">
          ${fixD(data.realisedAmountHistory, data.valuePrecision)}
          ${data.bond}
        </span>
      </div>`;
      return template;
    ...
    elementClick(type, id) {
      ...
      // 限价平仓
      if (type === 'closeTwo') {
        if (this.limitPriceObj[id]) {
          this.orderData.positions.forEach((item) => {
            if (id === item.id) {
              const obj = item;
              obj.indexPrice = this.limitPriceObj[id];
              obj.orderType = 1;
              this.close(obj);
            }
          });
        } else {
          this.$bus.$emit('tip', { text: this.$t('contract.pleprice'), type: 'error' });
        }
      }
      // 限价平仓文本框输入事件
      if (type === 'inputValueChane') {
        // 显示修改保证金的弹框
        const Dat = Object.keys(id);
        let pricePrecisions = 4;
        this.orderData.positions.forEach((item) => {
          if (Number(item.id) === Number(Dat[0])) {
            pricePrecisions = item.pricePrecision;
          }
        });
        this.limitPriceObj[Dat[0]] = fixInput(id[Dat[0]], pricePrecisions);
        this.formPosition(this.orderData.positions);
      }
      // 取消委托
      if (type === 'cancel') {
    ...
    lineClickEveent(data) {
      if (this.is_more_position === 1) {
        this.orderData.positions.forEach((item) => {
          if (Number(item.id) === data.id) {
            this.currenSymboltDataName = item.symbol;
            this.currenSymboltDataId = item.id;
            this.$bus.$emit('CO_CURRENT_TODER_DATA', item);
            if (this.symbolCurrent !== item.series) {
              this.symbolCurrent = item.series;
              this.$bus.$emit('ON_MARKET_SWITCH_ORDER', item.series);
            }
            if (this.marketCurrent !== item.symbol) {
              this.marketCurrent = item.symbol;
              this.$bus.$emit('ON_SYMBOL_SWITCH', item.symbol);
            }
          }
        });
      }
