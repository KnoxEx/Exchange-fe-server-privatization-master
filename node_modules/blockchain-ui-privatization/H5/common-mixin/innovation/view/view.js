import {
  fixD, formatTime, fixInput, nul, fixFloat, thousandsComma,
} from '@/utils';
...
export default {
  ...
  data() {
    return {
      ...
      navTab: [
        {
          name: this.$t('innov.info'),
          index: 2,
        },
        {
          name: this.$t('innov.Project_introduction'),
          index: 1,
        },
      ],
    };
  ...
  computed: {
    ...
    submitDisabled() {
      ...
      if (
        this.pageData.u_holdSymbol_amount >= this.pageData.holdAmount
    ...
    getData() {
      this.axios({
        ...
        if (data.code === '0') {
          ...
          this.bannerImg = data.data.h5Banner;
        }
      });
    },
    // goTrade
